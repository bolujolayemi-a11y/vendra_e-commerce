...
export default function PublicStore() {
  ...
  // --- REVISED LOGIC FOR SECTIONS ---
  
  // 1. Manual Clearance Section (Takes priority)
  const clearanceProducts = products.filter(p => p.is_clearance === true);

  // 2. New Arrivals (The 4 most recent items that are NOT clearance)
  const nonClearance = products.filter(p => !p.is_clearance);
  const newArrivals = nonClearance.slice(0, 4);

  // 3. General Stock (Everything else that isn't in the "New Arrivals" top 4)
  const generalProducts = nonClearance.slice(4);
  ...
  return (
    <div className="min-h-screen bg-white text-black selection:bg-black selection:text-white pb-40">
      ...
      <header className="pt-32 pb-20 px-6 text-center border-b border-gray-50">
        <div className="max-w-4xl mx-auto space-y-6">
            ...
            <p className="text-gray-400 font-bold uppercase text-[10px] tracking-[0.5em]">
                {store?.description || "High-Quality Materials • Vendra Store"}
            </p>
        </div>
      </header>

      {/* NEW ARRIVALS (Automatic Top 4) */}
      ...
      {/* CLEARANCE (Manual Toggle) */}
      {clearanceProducts.length > 0 && (
        <section className="bg-red-50/50 py-24 px-6 border-y border-red-100">
          <div className="max-w-7xl mx-auto">
              <div className="flex items-center gap-4 mb-12">
                  ...
                  <div className="h-[1px] flex-1 bg-red-200"></div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
      ...
      {/* GENERAL STOCK (Everything else) */}
      {generalProducts.length > 0 && (
        <section className="max-w-7xl mx-auto px-6 py-24">
          <div className="flex items-center gap-4 mb-12">
              <LayoutGrid size={20} className="text-gray-400" />
              <h2 className="text-2xl font-black uppercase tracking-tighter text-gray-400">General Stock</h2>
      ...
      {/* CART & WHATSAPP (Keep your existing code for these components) */}
      {totalItems > 0 && (
